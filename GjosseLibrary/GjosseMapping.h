// Most of the code here is from the mapping.h library provided by the RObotic Systems module

// Include header and other needed things

#ifndef _GjosseMapping_h
#define _GjosseMapping_h
#include <EEPROM.h>
#include <Arduino.h>

const byte MAP_RESOLUTION = 25;
const byte MAP_DEFAULT_FEATURE = '#';
const byte MEMORY_START = 6;
...
void Mapper::resetMap()
{
    ...
    for (int i=0;i<MAP_RESOLUTION;i++)
    {
        ...
        {
            int eeprom_address = (i*MAP_RESOLUTION)+j+MEMORY_START;
...
void Mapper::updateMapFeature(byte feature, int y, int x)
{
    ...
    int eeprom_address = (x_index * MAP_RESOLUTION) + y_index + MEMORY_START;  
